    ...
    for mname in model_names:
        ...
        for cname in ckpt_names:
            ...
            for dataset_name, dataset_data in data.items():
                padder = InputPadder(
                    dataset_data["images"].shape, stride=model.output_stride
                )
                ...
                epe = torch.norm(
                    preds - dataset_data["flows"], p=2, dim=2, keepdim=True
                )
...
def _load_data() -> Dict[str, Dict[str, torch.Tensor]]:
    ...
    data["flyingchairs"] = {
        "images": [
            cv.imread(
                str(Path("tests/data/ptlflow/models/flyingchairs_00001_img1.ppm"))
            ),
            cv.imread(
                str(Path("tests/data/ptlflow/models/flyingchairs_00001_img2.ppm"))
            ),
        ],
        "flows": flow_utils.flow_read(
            Path("tests/data/ptlflow/models/flyingchairs_00001_flow.flo")
        ),
    }
    data["flyingchairs"]["valids"] = np.ones_like(
        data["flyingchairs"]["flows"][:, :, 0]
    )
    ...
    data["flyingthings3d"] = {
        "images": [
            cv.imread(
                str(Path("tests/data/ptlflow/models/flyingthings3d_0000000.png"))
            ),
            cv.imread(
                str(Path("tests/data/ptlflow/models/flyingthings3d_0000001.png"))
            ),
        ],
        "flows": flow_utils.flow_read(
            Path("tests/data/ptlflow/models/flyingthings3d_0000000.flo")
        ),
    }
    data["flyingthings3d"]["valids"] = np.ones_like(
        data["flyingthings3d"]["flows"][:, :, 0]
    )
    data["flyingthings3d"] = transform(data["flyingthings3d"])

    data["kitti"] = {
        "images": [
            cv.imread(str(Path("tests/data/ptlflow/models/kitti2015_000000_10.png"))),
            cv.imread(str(Path("tests/data/ptlflow/models/kitti2015_000000_11.png"))),
        ],
        "flows": flow_utils.flow_read(
            Path("tests/data/ptlflow/models/kitti2015_flow_000000_10.png")
        ),
    }
    ...
    data["sintel"] = {
        "images": [
            cv.imread(
                str(
                    Path(
                        "tests/data/ptlflow/models/sintel/training/clean/alley_1/frame_0001.png"
                    )
                )
            ),
            cv.imread(
                str(
                    Path(
                        "tests/data/ptlflow/models/sintel/training/clean/alley_1/frame_0002.png"
                    )
                )
            ),
        ],
        "flows": flow_utils.flow_read(
            Path(
                "tests/data/ptlflow/models/sintel/training/flow/alley_1/frame_0001.flo"
            )
        ),
    }
    ...
    for dataset_dict in data.values():
        for k in dataset_dict.keys():
            ...
            scale_factor = 0.66
            dataset_dict[k] = F.interpolate(
                dataset_dict[k], scale_factor=scale_factor, recompute_scale_factor=False
            )
            if k == "flows":
