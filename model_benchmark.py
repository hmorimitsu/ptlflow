...
def _init_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        'model', type=str, default='all', choices=['all', 'select']+get_list_of_available_models_list(),
    ...
    parser.add_argument(
        '--sleep_interval', type=float, default=0.0,
        help=('Number of seconds to sleep between each repetition'))
    parser.add_argument(
        '--input_size', type=int, nargs='+', default=(500, 1000),
        help=(
    ...
    parser.add_argument(
        '--output_path', type=str, default=str(Path('outputs/benchmark')),
        help=('Path to a directory where the outputs will be saved.'))
    parser.add_argument(
        '--final_speed_mode', type=str, choices=('avg', 'median', 'perc1', 'perc5', 'perc10'), default='perc10',
        help=(
            'How to obtain the final speed results.'
            'percX represents reporting the value at the X-th percentile.'
        ))
    parser.add_argument(
        '--final_memory_mode', type=str, choices=('avg', 'median', 'perc1', 'perc5', 'perc10', 'first'), default='first',
        help=(
            'How to obtain the final memory results.'
            'percX represents reporting the value at the X-th percentile.'
        ))
    parser.add_argument(
        '--plot_axes', type=str, nargs=2, choices=TABLE_KEYS[1:], default=None,
        help=(
            'Name of two measured parameters to create a scatter plot.'
        ))
    parser.add_argument(
        '--plot_log_x', action='store_true',
        help='If set, the X-axis of the plot will be in log-scale.')
    parser.add_argument(
        '--plot_log_y', action='store_true',
        help='If set, the Y-axis of the plot will be in log-scale.')
    parser.add_argument(
        '--fp16', action='store_true',
        help='If set, use half floating point precision.')
    ...
    for isize in range(0, len(args.input_size), 2):
        ...
        for mname in tqdm(model_names):
            try:
                ...
                inputs = {
                    'images': torch.rand(
                        1, 2, 3, make_divisible(input_size[0], model.output_stride),
                ...
                df = df.round(3)
                df.to_csv(output_path / f'model_benchmark-{args.model}.csv', index=False)
                save_plot(output_path, args.model, df, args.plot_axes, args.plot_log_x, args.plot_log_y)
...
def save_plot(
    ...
    if plot_axes is not None:
        ...
        fig = px.scatter(
            df, x=TABLE_KEYS_LEGENDS[xkey], y=TABLE_KEYS_LEGENDS[ykey], color=TABLE_LEGENDS[0], symbol=TABLE_LEGENDS[0], log_x=log_x, log_y=log_y,
        ...
        logging.info('Saved plot between %s and %s at: %s', plot_axes[0], plot_axes[1], out_path)
...
if __name__ == '__main__':
    ...
    if args.csv_path is None:
        ...
        save_plot(args.output_path, args.model, df, args.plot_axes, args.plot_log_x, args.plot_log_y)
