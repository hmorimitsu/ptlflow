...
def flow_read(
    ...
    format: str, optional
        Specify in what format the flow is read, accepted formats: "png", "flo", "pfm", "flo5", "kubric_png".
...
def read_kubric_flow(
    input_file: Union[str, pathlib.Path, IO],
    flow_min: Optional[float],
    flow_max: Optional[float],
) -> np.ndarray:
    ...
    input_file: str, pathlib.Path or IO
        Path of the file to read or file object.
    flow_min: float
        The minimum flow range value, generated by Kubric and stored in the file data_ranges.json
    flow_max: float
        The maximum flow range value, generated by Kubric and stored in the file data_ranges.json
