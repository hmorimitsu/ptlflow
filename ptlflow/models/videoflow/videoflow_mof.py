...
class VideoFlowMOF(BaseModel):
    ...
    def __init__(self,
        ...
        if self.args.Tfusion == 'stack':
            ...
            from .Networks.MOFNetStack.stack import SKUpdateBlock6_Deep_nopoolres_AllDecoder2
            self.update_block = SKUpdateBlock6_Deep_nopoolres_AllDecoder2(args=self.args, hidden_dim=128//hidden_dim_ratio)
        ...
        self.att = Attention(args=self.args, dim=128//hidden_dim_ratio, heads=1, max_pos_size=160, dim_head=128//hidden_dim_ratio)
    ...
    def add_model_specific_args(parent_parser=None):
        ...
        parser.add_argument('--cnet', type=str, choices=('twins', 'basicencoder'), default='twins')
        parser.add_argument('--fnet', type=str, choices=('twins', 'basicencoder'), default='twins')
        parser.add_argument('--no_pretrain', action='store_false', dest='pretrain')
        parser.add_argument('--corr_fn', type=str, choices=('default', 'efficient'), default='default')
    ...
    def forward(self, inputs, flow_init=None):
        ...
        forward_corr_fn = corr_fn(fmaps[:, 1:N-1, ...].reshape(B*(N-2), -1, H//down_ratio, W//down_ratio), fmaps[:, 2:N, ...].reshape(B*(N-2), -1, H//down_ratio, W//down_ratio), num_levels=self.args.corr_levels, radius=self.args.corr_radius)
        backward_corr_fn = corr_fn(fmaps[:, 1:N-1, ...].reshape(B*(N-2), -1, H//down_ratio, W//down_ratio), fmaps[:, 0:N-2, ...].reshape(B*(N-2), -1, H//down_ratio, W//down_ratio), num_levels=self.args.corr_levels, radius=self.args.corr_radius)
        ...
        for itr in range(self.args.decoder_depth):
            ...
            flow_predictions.append(torch.cat([forward_flow_up, backward_flow_up], dim=1))
