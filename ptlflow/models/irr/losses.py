...
def fbeta_score(y_true, y_pred, beta, eps=1e-8):
    ...
    return torch.mean(
        precision * recall / (precision * beta2 + recall + eps) * (1 + beta2)
    )
...
def f1_score_bal_loss(y_pred, y_true):
    ...
    fn = (
        -((1 - y_true) * torch.log((1 - y_pred) + eps)).sum(dim=2).sum(dim=2).sum(dim=1)
    )

    denom_tp = (
        y_true.sum(dim=2).sum(dim=2).sum(dim=1)
        + y_pred.sum(dim=2).sum(dim=2).sum(dim=1)
        + eps
    )
    denom_fn = (
        (1 - y_true).sum(dim=2).sum(dim=2).sum(dim=1)
        + (1 - y_pred).sum(dim=2).sum(dim=2).sum(dim=1)
        + eps
    )

    return (
        ((tp / denom_tp).sum() + (fn / denom_fn).sum())
        * y_pred.size(2)
        * y_pred.size(3)
        * 0.5
    )
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            ...
            for i, output_i in enumerate(outputs):
                ...
                total_loss = (
                    total_loss + self._weights[i] * epe_i.sum() / self._batch_size
                )
        ...
        if self.training:
            ...
            for ii, output_ii in enumerate(outputs_flo):
                ...
                for jj, output_ii_jj in enumerate(output_ii):
                    ...
                    total_loss = total_loss + self._weights[ii] * (
                        epe_f_ii.sum() + epe_b_ii.sum()
                    )
                    loss_dict["epe%i" % (ii + 2)] = (
                        epe_f_ii.mean() + epe_b_ii.mean()
                    ) / 2
...
class MultiScaleEPE_FlowNet_IRR_Occ(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            ...
            outputs_occ = [
                output_dict[key] for key in ["occ2", "occ3", "occ4", "occ5", "occ6"]
            ]
            ...
            for ii, output_ii in enumerate(outputs_flo):
                ...
                for jj, output_ii_jj in enumerate(output_ii):
                    flow_loss = (
                        flow_loss
                        + self._weights[ii]
                        * _elementwise_epe(output_ii_jj, target_ii).sum()
                    )

            for ii, output_ii in enumerate(outputs_occ):
                target_occ_f = _downsample2d_as(target_occ, output_ii[0])
                for jj, output_ii_jj in enumerate(output_ii):
                    occ_loss = occ_loss + self._weights[ii] * self.f1_score_bal_loss(
                        self.occ_activ(output_ii_jj), target_occ_f
                    )
            ...
            loss_dict["loss"] = (
                (flow_loss * f_l_w + occ_loss * o_l_w)
                / self._batch_size
                / self._num_iters
            )
            ...
            loss_dict["epe"] = _elementwise_epe(
                output_dict["flow1"], target_dict["flows"][:, 0]
            ).mean()
            loss_dict["F1"] = f1_score(
                target_dict["occs"][:, 0],
                torch.round(self.occ_activ(output_dict["occ1"])),
            )
        ...
        if self.training:
            ...
            for ii, output_ii in enumerate(outputs_flo):
                ...
                for jj, output_ii_jj in enumerate(output_ii):
                    ...
                    flow_loss = (
                        flow_loss
                        + self._weights[ii] * (epe_f_ii.sum() + epe_b_ii.sum()) * 0.5
                    )
            ...
            for ii, output_ii in enumerate(outputs_occ):
                ...
                for jj, output_ii_jj in enumerate(output_ii):
                    ...
                    occ_loss = (
                        occ_loss + self._weights[ii] * (bce_f_ii + bce_b_ii) * 0.5
                    )
...
class MultiScaleEPE_FlowNet_IRR_Bi_Occ_upsample(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            ...
            outputs_occ = [
                output_dict[key]
                for key in ["occ", "occ1", "occ2", "occ3", "occ4", "occ5", "occ6"]
            ]
            ...
            loss_dict["loss"] = (
                (flow_loss * f_l_w + occ_loss * o_l_w) / self._batch_size / num_iters
            )
        else:
            loss_dict["epe"] = _elementwise_epe(
                output_dict["flows"][:, 0], target_dict["flows"][:, 0]
            ).mean()
            loss_dict["F1"] = f1_score(
                target_dict["occs"][:, 0],
...
class MultiScaleEPE_PWC_Bi(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            ...
            total_loss = 0
            for i, output_i in enumerate(outputs):
                epe_i_f = _elementwise_epe(
                    output_i[0], _downsample2d_as(target_f, output_i[0])
                )
                epe_i_b = _elementwise_epe(
                    output_i[1], _downsample2d_as(target_b, output_i[1])
                )
                total_loss = total_loss + self._weights[i] * (
                    epe_i_f.sum() + epe_i_b.sum()
                )
        ...
        if self.training:
            ...
            for i, output_i in enumerate(output_flo):
                flow_loss = (
                    ...
                    + self._weights[i]
                    * _elementwise_epe(
                        output_i, _downsample2d_as(target_flo, output_i)
                    ).sum()
                )

            for i, output_i in enumerate(output_occ):
                output_occ = self.occ_activ(output_i)
                occ_loss = occ_loss + self._weights[i] * self.f1_score_bal_loss(
                    output_occ, _downsample2d_as(target_occ, output_occ)
                )
        ...
        if self.training:
            ...
            for i, output_i in enumerate(output_flo):
                flow_loss = (
                    ...
                    + self._weights[i]
                    * _elementwise_epe(
                        output_i[0], _downsample2d_as(target_flo_f, output_i[0])
                    ).sum()
                )
                flow_loss = (
                    flow_loss
                    + self._weights[i]
                    * _elementwise_epe(
                        output_i[1], _downsample2d_as(target_flo_b, output_i[1])
                    ).sum()
                )
            ...
            for i, output_i in enumerate(output_occ):
                ...
                occ_loss = occ_loss + self._weights[i] * self.f1_score_bal_loss(
                    output_occ_f, _downsample2d_as(target_occ_f, output_occ_f)
                )
                occ_loss = occ_loss + self._weights[i] * self.f1_score_bal_loss(
                    output_occ_b, _downsample2d_as(target_occ_b, output_occ_b)
                )
            ...
            loss_dict["loss"] = (flow_loss * f_l_w + occ_loss * o_l_w) / (
                2 * self._batch_size
            )
        ...
        if self.training:
            ...
            for ii, output_ii in enumerate(output_flo):
                ...
                for jj in range(0, len(output_ii) // 2):
                    loss_ii = (
                        ...
                        + _elementwise_epe(
                            output_ii[2 * jj],
                            _downsample2d_as(target_flo_f, output_ii[2 * jj]),
                        ).sum()
                    )
                    loss_ii = (
                        loss_ii
                        + _elementwise_epe(
                            output_ii[2 * jj + 1],
                            _downsample2d_as(target_flo_b, output_ii[2 * jj + 1]),
                        ).sum()
                    )
            ...
            for ii, output_ii in enumerate(output_occ):
                ...
                for jj in range(0, len(output_ii) // 2):
                    ...
                    loss_ii = loss_ii + self.f1_score_bal_loss(
                        output_occ_f, _downsample2d_as(target_occ_f, output_occ_f)
                    )
                    loss_ii = loss_ii + self.f1_score_bal_loss(
                        output_occ_b, _downsample2d_as(target_occ_b, output_occ_b)
        ...
        if self.training:
            ...
            for ii, output_ii in enumerate(output_occ):
                ...
                for jj in range(0, len(output_ii) // 2):
                    output_occ_f = self.occ_activ(output_ii[2 * jj])
                    output_ii[2 * jj + 1] = output_ii[2 * jj + 1].detach()
                    loss_ii = loss_ii + self.occ_loss_bce(
                        output_occ_f, _downsample2d_as(target_occ_f, output_occ_f)
                    )
        ...
        if self.training:
            ...
            for ii, output_ii in enumerate(output_flo):
                ...
                for jj in range(0, len(output_ii) // 2):
                    valid_epe = (
                        _elementwise_robust_epe_char(
                            _upsample2d_as(output_ii[2 * jj], target_flo_f),
                            target_flo_f,
                        )
                        * valid_mask
                    )
                    ...
                    for bb in range(0, b):
                        valid_epe[bb, ...][valid_mask[bb, ...] == 0] = valid_epe[
                            bb, ...
                        ][valid_mask[bb, ...] == 0].detach()
                        norm_const = h * w / (valid_mask[bb, ...].sum())
                        loss_ii = (
                            loss_ii
                            + valid_epe[bb, ...][valid_mask[bb, ...] != 0].sum()
                            * norm_const
                        )
            ...
            flow_epe = (
                _elementwise_epe(output_dict["flows"][:, 0], target_dict["flows"][:, 0])
                * valid_mask
            )

            epe_per_image = (flow_epe.view(b, -1).sum(1)) / (
                valid_mask.view(b, -1).sum(1)
            )
            loss_dict["epe"] = epe_per_image.mean()

            outlier_epe = (
                (flow_epe > 3).float()
                * ((flow_epe / flow_gt_mag) > 0.05).float()
                * valid_mask
            )
            outlier_per_image = (outlier_epe.view(b, -1).sum(1)) / (
                valid_mask.view(b, -1).sum(1)
            )
