...
class MpiSintel(FlowDataset):
    def __init__(
        self, aug_params=None, split="training", root="data/sintel", dstype="clean"
    ):
...
class FlyingChairs(FlowDataset):
    def __init__(self, aug_params=None, split="train", root="data/FlyingChairs/data"):
...
class LayeredFlow(data.Dataset):
    def __init__(
        self, root="data/public_layeredflow_benchmark", downsample=4, split="val"
    ):
        super(LayeredFlow, self).__init__()

        self.downsample = downsample
        self.path = os.path.join(root, split)
        self.data_list = []
        for scene in os.listdir(self.path):
            self.data_list.extend([(scene, 0), (scene, 1)])

    def __len__(self):
        return len(self.data_list)

    def __getitem__(self, index):
        scene = self.data_list[index][0]
        stereo = self.data_list[index][1]
        path = osp.join(self.path, scene)

        gt = pickle.load(open(osp.join(path, "gt.pickle"), "rb"))
        optical_points = gt["optical_points"]
        annotations = gt["annotations"]

        image1 = osp.join(path, f"0_{stereo}.png")
        image2 = osp.join(path, f"3_{stereo}.png")
        img1 = frame_utils.read_gen(image1)
        img2 = frame_utils.read_gen(image2)
        if self.downsample > 1:
            width, height = img1.size
            img1 = img1.resize((width // self.downsample, height // self.downsample))
            img2 = img2.resize((width // self.downsample, height // self.downsample))
        img1 = np.array(img1).astype(np.uint8)[..., :3]
        img2 = np.array(img2).astype(np.uint8)[..., :3]
        img1 = torch.from_numpy(img1).permute(2, 0, 1).float()
        img2 = torch.from_numpy(img2).permute(2, 0, 1).float()
        ...
        material_map = {"Diffuse": 0, "Transparent": 1, "Reflective": 2}
        for point1, point2 in zip(optical_points[stereo], optical_points[stereo + 2]):
            point1_index = (point1[0], point1[1])
            point2_index = (point2[0], point2[1])
            y1, x1 = point1[2]
            y2, x2 = point2[2]
            assert point1_index == point2_index
            point_index = point1_index

            valid = annotations[point_index][0]
            material_name = annotations[point_index][1]
            layer_number = annotations[point_index][2]
            if not valid:
                continue

            ix1, iy1 = round(x1 / self.downsample), round(y1 / self.downsample)
            if ix1 < 0 or ix1 >= h or iy1 < 0 or iy1 >= w:
                continue

            coords.append((ix1, iy1))
            flows.append(((y2 - y1) / self.downsample, (x2 - x1) / self.downsample))
            materials.append(material_map[material_name])
            layers.append(layer_number)

        assert len(coords) == len(flows)
        assert len(coords) == len(materials)
        assert len(coords) == len(layers)
...
class SpringFlowDemoDataset(FlowDataset):
    """
    Dataset class for Spring optical flow dataset.
    For train, this dataset returns image1, image2, flow and a data tuple (framenum, scene name, left/right cam, FW/BW direction).
    For test, this dataset returns image1, image2 and a data tuple (framenum, scene name, left/right cam, FW/BW direction).
    ...
    def __init__(
        ...
        super(SpringFlowDemoDataset, self).__init__(aug_params)
        ...
        for scene in sorted(scenes):
            ...
            for idx in range(len(images) - 1):
                ...
                self.flow_list.append(
                    osp.join(scene, "frames/Flow3D/camera_0", f"Flow3D_{ID}.npy")
                )
...
class TartanAir(FlowDataset):
    ...
    def __init__(self, aug_params=None, root="data/Tartanair/", split="train"):
    ...
    def _build_dataset(self):
        ...
        for scene in sorted(scenes):
            if self.split == "train" and (
                "westerndesert" in scene or "soulcity" in scene
            ):
                continue
            elif self.split == "val" and (
                "westerndesert" not in scene and "soulcity" not in scene
            ):
                continue
...
def fetch_dataloader(args, rank=0, world_size=1, use_ddp=False):
    ...
    if args.dataset == "chairs":
        ...
        train_dataset = FlyingChairs(
            aug_params, split="training", root=args.paths["chairs"]
        )
        ...
        clean_dataset = FlyingThings3D(
            aug_params, dstype="frames_cleanpass", root=args.paths["things"]
        )
        final_dataset = FlyingThings3D(
            aug_params, dstype="frames_finalpass", root=args.paths["things"]
        )
        ...
        sintel_clean = MpiSintel(
            aug_params, split="training", dstype="clean", root=args.paths["sintel"]
        )
        sintel_final = MpiSintel(
            aug_params, split="training", dstype="final", root=args.paths["sintel"]
        )
        ...
        train_dataset = SpringFlowDataset(
            aug_params, subsample_groundtruth=True, root=args.paths["spring"]
        )
