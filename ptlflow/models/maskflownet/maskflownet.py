...
def deformable_conv(
    in_planes, out_planes, kernel_size=3, strides=1, padding=1, use_bias=True
):
    return ops.DeformConv2d(
        in_planes, out_planes, kernel_size, strides, padding, bias=use_bias
    )
...
class MaskFlownet_S(BaseModel):
    ...
    def __init__(self, args):
        ...
        self.conv5f = conv(
            16, 128, kernel_size=3, stride=1, padding=1, activation=False
        )
    ...
    def forward(self, inputs, skip_preprocess=False):
        ...
        predictions = [
            flow.flip(1) * self.scale for flow in [flow6, flow5, flow4, flow3, flow2]
        ]
        occlusion_masks = []
    ...
    def forward(self, output, target):
        ...
        if self.match == "upsampling":
            for p, w, s in zip(predictions, self.weights, self.scales):
                losses += (
                    EpeLossWithMask(eps=self.eps, q=self.q)(Upsample(p, s), flow, mask)
                    * w
                )
        elif self.match == "downsampling":
            for p, w, s in zip(predictions, self.weights, self.scales):
                losses += (
                    EpeLossWithMask(eps=self.eps, q=self.q)(
                        p, Downsample(flow, s), Downsample(mask, s)
                    )
                    * w
                )
        else:
