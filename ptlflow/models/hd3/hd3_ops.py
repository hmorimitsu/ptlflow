...
def downsample_flow(flo, scale_factor):
    ...
    if flo.size(1) == 2:
        # dense format
        flo = F.interpolate(
            flo, scale_factor=scale_factor, mode="bilinear", align_corners=True
        )
        mask = torch.ones(
...
def resize_dense_vector(vec, des_height, des_width):
    ...
    if vec.size(1) == 1:
        ...
        vec = torch.stack(
            [vec[:, 0, :, :] * ratio_width, vec[:, 1, :, :] * ratio_height], dim=1
        )
    return vec
...
def _prob2cornerflow(prob, normalize=True):
        ...
        if version.parse(torch.__version__) >= version.parse("1.8.0"):
            return torch.cat(
                [
                    ...
                    torch.div(ind, d, rounding_mode="floor")
                    - torch.div(d, 2, rounding_mode="floor"),
                ],
                1,
            )
        else:
    ...
    if version.parse(torch.__version__) >= version.parse("1.8.0"):
        indice += torch.div(
            indice, (d - 1), rounding_mode="floor"
