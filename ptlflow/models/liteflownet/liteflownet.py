...
class Matching(nn.Module):
    def __init__(self, level: int, num_levels: int = 5, div_flow: float = 20.0) -> None:
        ...
        self.corr = SpatialCorrelationSampler(
            ...
            padding=0,
            stride=corr_stride,
            dilation_patch=corr_stride,
        )

        self.warp = WarpingLayer()

    def forward(
        self, feats: torch.Tensor, flow: Optional[torch.Tensor]
    ) -> torch.Tensor:
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for i in range(self.num_levels):
            ...
            if i == (self.num_levels - 1):
                feats2 = self.feat2_conv(feats2.view(-1, *feats2.shape[2:])).view(
                    *feats2.shape[:2], -1, *feats2.shape[3:]
                )
