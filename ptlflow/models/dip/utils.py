...
class InputPadder:
    ...
    def __init__(self, dims, size_base=16, mode="sintel"):
        self.ht, self.wd = dims[-2:]
        pad_ht = (((self.ht // size_base) + 1) * size_base - self.ht) % size_base
        pad_wd = (((self.wd // size_base) + 1) * size_base - self.wd) % size_base
        if mode == "sintel":
            self._pad = [
                pad_wd // 2,
                pad_wd - pad_wd // 2,
