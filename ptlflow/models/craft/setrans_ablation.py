...
def positionalencoding2d(pos_embed_dim, height, width):
    ...
    div_term = torch.exp(
        torch.arange(0.0, pos_embed_dim, 2) * -(math.log(10000.0) / pos_embed_dim)
    )
    pos_w = torch.arange(0.0, width).unsqueeze(1)
    pos_h = torch.arange(0.0, height).unsqueeze(1)
    pe[0:pos_embed_dim:2, :, :] = (
        torch.sin(pos_w * div_term).transpose(0, 1).unsqueeze(1).repeat(1, height, 1)
    )
    pe[1:pos_embed_dim:2, :, :] = (
        torch.cos(pos_w * div_term).transpose(0, 1).unsqueeze(1).repeat(1, height, 1)
    )
    pe[pos_embed_dim::2, :, :] = (
        torch.sin(pos_h * div_term).transpose(0, 1).unsqueeze(2).repeat(1, 1, width)
    )
    pe[pos_embed_dim + 1 :: 2, :, :] = (
...
class RandPosEmbedder(nn.Module):
    def __init__(self, pos_dim, pos_embed_dim, shape, affine):
        ...
        self.pos_embed_norm_layer = nn.LayerNorm(
...
class SinuPosEmbedder(nn.Module):
    def __init__(self, pos_dim, pos_embed_dim, shape, affine):
        ...
        self.pos_embed = self.pos_embed.cuda().reshape(
            (shape[0] * shape[1], pos_embed_dim)
        )
...
class ZeroEmbedder(nn.Module):
    ...
    def forward(self, pos_normed):
        B, N, D = pos_normed.shape
        zero_pos_embed = torch.zeros(
            B, N, self.pos_embed_dim, requires_grad=False
        ).cuda()
...
class MultiHeadFeatTrans(nn.Module):
    def __init__(self, config, name):
        ...
        print(
            "%s: pool_modes_feat=concat, trans_output_type=%s"
    ...
    def forward(self, input_feat, attention_probs, attention_scores):
        ...
        if self.apply_attn_early:
            # shape_4d: [B0, 8, 1024, 50]
            shape_4d = (
                mm_first_feat.shape[0],
                self.num_modes,
                self.feat_dim_onehead,
                mm_first_feat.shape[2],
            )
            ...
            mm_first_feat_fusion_3d = mm_first_feat_fusion.permute(
                [0, 1, 3, 2]
            ).reshape(mm_first_feat.shape)
