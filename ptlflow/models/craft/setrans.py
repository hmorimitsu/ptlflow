...
class SETransConfig(object):
    ...
    def update_config(self, args):
        ...
        self.try_assign(
            ...
            "num_modes",
            "trans_output_type",
            "base_initializer_range",
            "pos_code_type",
            "ablate_multihead",
            "attn_clip",
            "attn_diag_cycles",
            "tie_qk_scheme",
            "feattrans_lin1_idbias_scale",
            "qk_have_bias",
            "v_has_bias",
...
def tie_qk(module):
    if (
        isinstance(module, CrossAttFeatTrans)
        and module.tie_qk_scheme != "none"
        and module.tie_qk_scheme != None
    ):
        module.tie_qk()
...
class MMPrivateOutput(nn.Module):
    def __init__(self, config):
        ...
        self.group_linear = nn.Conv1d(
            feat_dim_allmode, feat_dim_allmode, 1, groups=self.num_modes
        )
        self.resout_norm_layer = nn.LayerNorm(
            self.feat_dim, eps=1e-12, elementwise_affine=True
        )
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class ExpandedFeatTrans(nn.Module):
    def __init__(self, config, name):
        ...
        self.first_linear = nn.Linear(
            self.in_feat_dim, self.feat_dim_allmode, bias=config.v_has_bias
        )
        ...
        self.drop_path = (
            DropPath(config.drop_path_prob)
            if config.drop_path_prob > 0.0
            else nn.Identity()
        )

        print0(
            "{}: v_has_bias: {}, has_FFN: {}, has_input_skip: {}".format(
        ...
        if self.pool_modes_feat == "softmax":
            ...
            self.feat_softaggr = LearnedSoftAggregate(
                agg_basis_feat_dim, group_dim=1, keepdim=self.pool_modes_keepdim
            )
        ...
        if self.has_input_skip:
            ...
            self.skip_layer_norm = nn.LayerNorm(
                self.feat_dim, eps=1e-12, elementwise_affine=False
            )
    ...
    def add_identity_bias(self):
        if self.config.feattrans_lin1_idbias_scale > 0:
            ...
            identity_weight = (
                torch.diag(torch.ones(self.feat_dim))
                * self.base_initializer_range
            ...
            self.first_linear.weight.data[: self.feat_dim, : self.feat_dim] = (
                self.first_linear.weight.data[: self.feat_dim, : self.feat_dim] * 0.5
                + identity_weight
            )
    ...
    def forward(self, input_feat, attention_probs):
        ...
        mm_first_feat_fusion_3d = mm_first_feat_fusion.transpose(2, 3).reshape(
            B, M * F, U1
        )
...
class CrossAttFeatTrans(SETransInitWeights):
    def __init__(self, config, name):
        ...
        self.query = nn.Linear(
            self.in_feat_dim, self.att_size_allmode, bias=config.qk_have_bias
        )
        self.key = nn.Linear(
            self.in_feat_dim, self.att_size_allmode, bias=config.qk_have_bias
        )
        ...
        print0(
            "{}: in_feat_dim: {}, feat_dim: {}, modes: {}, qk_have_bias: {}".format(
                self.name,
                self.in_feat_dim,
                self.feat_dim,
                self.num_modes,
    ...
    def add_identity_bias(self):
        identity_weight = (
            torch.diag(torch.ones(self.attention_mode_dim))
            * self.base_initializer_range
    ...
    def forward(self, query_feat, key_feat=None, pos_biases=None, attention_mask=None):
        ...
        )  # [B0, 4, U1, 448] [B0, 4, 448, U2]
        attention_scores = attention_scores / math.sqrt(
            self.attention_mode_dim
        )  # [B0, 4, U1, U2]
        ...
        if curr_max_attn > self.attn_clip:
            attention_scores = torch.clamp(
                attention_scores, -self.attn_clip, self.attn_clip
            )
            self.clamp_count += 1
        ...
        if self.training:
            if self.call_count % self.attn_diag_cycles == 0:
                print0(
                    "max-attn: {:.2f}, avg-attn: {:.2f}, clamp-count: {}".format(
                        self.max_attn, curr_avg_attn, self.clamp_count
                    )
                )
                self.max_attn = 0
                self.clamp_count = 0
...
class SelfAttVisPosTrans(nn.Module):
    ...
    def forward(self, x):
        ...
        x_trans = self.setrans(
            x_vispos, pos_biases=pos_biases, attention_mask=attn_mask
        )
...
class LearnedSinuPosEmbedder(nn.Module):
    def __init__(self, pos_dim, pos_embed_dim, omega=1, affine=True):
        ...
        self.pos_mix_norm_layer = nn.LayerNorm(
            self.pos_embed_dim, eps=1e-12, elementwise_affine=affine
        )
        self.omega = omega
    ...
    def forward(self, pos_normed):
        ...
        pos_embed_mix = torch.stack((pos_embed_sin, pos_embed_cos), dim=3).view(
            pos_embed0.shape
        )
...
class SETransInputFeatEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.comb_norm_layer = nn.LayerNorm(
            self.feat_dim, eps=1e-12, elementwise_affine=False
        )
        ...
        if config.pos_code_type == "lsinu":
            self.pos_coder = LearnedSinuPosEmbedder(
                config.pos_dim, self.pos_embed_dim, omega=1, affine=False
            )
        elif config.pos_code_type == "rand":
            self.pos_coder = RandPosEmbedder(
                config.pos_dim, self.pos_embed_dim, shape=(36, 36), affine=False
            )
        elif config.pos_code_type == "sinu":
            self.pos_coder = SinuPosEmbedder(
                config.pos_dim, self.pos_embed_dim, shape=(36, 36), affine=False
            )
    ...
    def pos_code_lookup_cache(self, vis_feat_shape, dtype, device, voxels_pos_normed):
        if self.pos_code_type == "bias":
            ...
            if (
                self.training
                or self.cached_pos_code is None
                or self.cached_feat_shape != voxels_pos_normed.shape
            ):
