...
class DICL_MODULE(nn.Module):
    ...
    def __init__(self):
        ...
        self.match = nn.Sequential(
            ...
            BasicConv(
                64, 32, kernel_size=4, padding=1, stride=2, deconv=True
            ),  # up by 1/2
...
class DAP(nn.Module):
    def __init__(self, md=3, dap_by_temperature=False):
        ...
        dimC = (2 * md + 1) ** 2
        self.dap_layer = BasicConv(
            dimC, dimC, kernel_size=1, padding=0, stride=1, bn=False, relu=False
        )
        if self.dap_by_temperature:
            self.dap_layer = BasicConv(
                dimC, 1, kernel_size=1, padding=0, stride=1, bn=False, relu=False
            )
...
class DICLBase(BaseModel):
    ...
    def forward(self, inputs):
        ...
        flow_out = F.interpolate(
            flow2, size=x.shape[-2:], mode="bilinear", align_corners=True
        )[:, None]
    ...
    def compute_cost(self, x, y, matchnet, maxU, maxV):
        ...
        with torch.cuda.device_of(x):
            ...
            cost = (
                x.new()
                .resize_(x.size()[0], 2 * c, 2 * maxU + 1, 2 * maxV + 1, height, width)
                .zero_()
            )
...
class BasicConv(nn.Module):
    def __init__(
        self, in_channels, out_channels, deconv=False, bn=True, relu=True, **kwargs
    ):
...
class Conv2x(nn.Module):
    def __init__(
        self, in_channels, out_channels, deconv=False, concat=True, bn=True, relu=True
    ):
