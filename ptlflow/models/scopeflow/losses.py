...
class MultiScaleEPE_PWC_Bi_Occ_upsample_Sintel(nn.Module):
    ...
    def _get_flow_loss(self, output_flo, target_flo_f):
        ...
        for ii, output_ii in enumerate(output_flo):
            loss_ii = 0
            for jj in range(0, len(output_ii) // 2):
                cur_epe = _elementwise_robust_epe_char(
                    output_ii[2 * jj], _downsample2d_as(target_flo_f, output_ii[2 * jj])
                )
                if self.loss_perc:
                    tmin = torch.Tensor(
                        [self.perc(ten.flatten(), [self.min_p]) for ten in cur_epe]
                    )
                    # tmin.requires_grad = False
                    tmax = torch.Tensor(
                        [self.perc(ten.flatten(), [self.max_p]) for ten in cur_epe]
                    )
                    # tmax.requires_grad = False
                    cur_epe[cur_epe > tmax.view(-1, 1, 1, 1).cuda()] = 0.0
                    cur_epe[cur_epe < tmin.view(-1, 1, 1, 1).cuda()] = 0.0
                    tmin.detach()
                    tmax.detach()
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            ...
            # Mask with occ
            if self.seploss:
                output_flo, target_flo_f = self.mask_flow(
                    output_flo, target_flo_f, output_occ
                )
