...
def compute_sparse_corr(fmap1, fmap2, k=32):
    ...
    corr_sp = torch.einsum("bcn,bckn->bkn", fmap1, fmap2).contiguous() / torch.sqrt(
...
class SCVQuarter(SCVBase):
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.fnet = BasicEncoderQuarter(
            output_dim=256, norm_fn="instance", dropout=False
        )
        ...
        for itr in range(self.args.iters):
            with torch.no_grad():
                ...
                for i in range(5):
                    ...
                    mask = (coords1_sp[:, 0].abs() <= search_range) & (
                        coords1_sp[:, 1].abs() <= search_range
                    )
                    batch_ind = batch_index_cv.permute(0, 2, 3, 1).repeat(1, 4, 1, 1)[
                        mask
                    ]
            ...
            sparse_tensor_pyramid = [
                torch.sparse.FloatTensor(
                    coords_sp.t().long(),
                    corr_resample,
                    torch.Size([B, H1, W1, 9, 9, 1]),
                ).coalesce()
                for coords_sp, corr_resample in zip(
                    coords_sparse_pyramid, corr_val_pyramid
                )
            ]

            corr = torch.cat(
                [sp.to_dense().view(B, H1, W1, -1) for sp in sparse_tensor_pyramid],
                dim=3,
            ).permute(0, 3, 1, 2)
