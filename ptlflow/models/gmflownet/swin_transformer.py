...
class NeighborWindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(
                ((self.n_win + 1) * window_size[0] - 1)
                * ((self.n_win + 1) * window_size[1] - 1),
                num_heads,
            )
        )  # 2*Wh-1 * 2*Ww-1, nH
        ...
        coords_neig = torch.stack(
            torch.meshgrid([coords_h_neig, coords_w_neig], indexing="ij")
        )  # 2, Wh, Ww

        coords_flat = torch.flatten(coords, 1)  # 2, Wh*Ww
        coords_neig_flat = torch.flatten(coords_neig, 1)  # 2, (n_win*Wh)*(n_win*Ww)
        relative_coords = (
            coords_flat[:, :, None] - coords_neig_flat[:, None, :]
        )  # 2, Wh*Ww, (n_win*Wh)*(n_win*Ww)
        relative_coords = relative_coords.permute(
            1, 2, 0
        ).contiguous()  # Wh*Ww,(n_win*Wh)*(n_win*Ww), 2
        relative_coords[:, :, 0] += (
            self.n_win * self.window_size[0] - 1
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = (
            self.Wq(q)
            .reshape(B_, N_q, self.num_heads, dim_per_head)
            .permute(0, 2, 1, 3)
        )
        k = (
            self.Wk(k)
            .reshape(B_, N_kv, self.num_heads, dim_per_head)
            .permute(0, 2, 1, 3)
        )
        v = (
            self.Wv(v)
        ...
        relative_position_bias = self.relative_position_bias_table[
            ...
            self.window_size[0] * self.window_size[1],
            self.n_win * self.window_size[0] * self.n_win * self.window_size[1],
            -1,
        )  # Wh*Ww,(n_win*Wh)*(n_win*Ww),nH
        relative_position_bias = relative_position_bias.permute(
            2, 0, 1
        ).contiguous()  # nH, Wh*Ww, (n_win*Wh)*(n_win*Ww)
        attn = attn + relative_position_bias.unsqueeze(0)

        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N_q, N_kv) + mask.unsqueeze(
                1
            ).unsqueeze(0)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        k = (
            self.Wk(k)
            .reshape(B, N_kv, self.num_heads, dim_per_head)
            .permute(0, 2, 1, 3)
        )
        v = (
            self.Wv(v)
...
class POLATransBlock(nn.Module):
    ...
    def forward(self, x, H, W, attn_mask=None):
        ...
        key_val = F.pad(
            x, (0, 0, pad_size, pad_size, pad_size, pad_size)
        )  # B, H'+2*1*win, W'+2*1*win, C
        key_val = F.unfold(
            key_val.permute(0, 3, 1, 2),
            self.n_win * self.window_size,
            stride=self.window_size,
        )
        key_val = (
            key_val.permute(0, 2, 1)
            .reshape(-1, C, (self.n_win * self.window_size) ** 2)
            .permute(0, 2, 1)
        )  # (B*num_win, (3*3)*win_size*win_size, C)

        # Local attention feature
        attn_windows = self.attn(
            x_win, key_val, key_val, mask=attn_mask
        )  # nW*B, window_size*window_size, C
...
class MixAxialPOLABlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = norm_layer(dim)
        self.localAttn = NeighborWindowAttention(
            self.local_chl,
            window_size=to_2tuple(self.window_size),
            num_heads=self.local_head,
        ...
        # for axial attention
        self.vertiAttn = MultiHeadAttention(
            self.axis_chl,
            num_heads=self.axis_head,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
            attn_drop=attn_drop,
            proj_drop=drop,
        ...
        self.horizAttn = MultiHeadAttention(
            self.axis_chl,
            num_heads=self.axis_head,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
    ...
    def forward(self, x, H, W, attn_mask=None):
        ...
        x_local, x_horiz, x_verti = torch.split(
            x, [self.local_chl, self.axis_chl, self.axis_chl], dim=-1
        )
        ...
        # partition windows
        x_windows = window_partition(
            x_local, self.window_size
        )  # nW*B, window_size, window_size, C1
        x_windows = x_windows.view(
            -1, self.window_size * self.window_size, self.local_chl
        )  # nW*B, window_size*window_size, C1

        # pad and unfold
        pad_size = self.neig_win_num * self.window_size
        key_val = F.pad(
            x_local, (0, 0, pad_size, pad_size, pad_size, pad_size)
        )  # B, H'+2*1*win, W'+2*1*win, C
        key_val = F.unfold(
            key_val.permute(0, 3, 1, 2),
            self.n_win * self.window_size,
        ...
        key_val = (
            key_val.permute(0, 2, 1)
            .reshape(-1, self.local_chl, (self.n_win * self.window_size) ** 2)
            .permute(0, 2, 1)
        )  # (B*num_win, (3*3)*win_size*win_size, C)

        # Local attention feature
        attn_windows = self.localAttn(
            x_windows, key_val, key_val, mask=attn_mask
        )  # nW*B, window_size*window_size, C1
        ...
        # Vertical update
        x_verti = x_verti.transpose(1, 2).reshape(
            -1, H, self.axis_chl
        )  # B, W, H, C3 -> (B*W), H, C3
...
class POLAUpdate(nn.Module):
    ...
    def forward(self, x):
        ...
        img_mask = F.pad(
            img_mask,
            (0, 0, pad_extra, pad_r + pad_extra, pad_extra, pad_b + pad_extra),
            mode="constant",
            value=float(-100.0),
        )
        ...
        mask_windows = F.unfold(
            img_mask.permute(0, 3, 1, 2), n_win * self.win_size, stride=self.win_size
        )
        mask_windows = mask_windows.permute(0, 2, 1).reshape(
            -1, (n_win * self.win_size) ** 2
        )  # (num_win, (3*3)*win_size*win_size)
        attn_mask = mask_windows.unsqueeze(1).repeat(
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                self.x_list.append(
                    x.view(-1, H, W, self.num_feature).permute(0, 3, 1, 2).contiguous()
                )
