...
class VCNLoss(nn.Module):
    ...
    def forward(self, preds, inputs):
        ...
        mask = (
            (inputs["valids"][:, 0, 0] == 1)
            & (flowl0[:, 0].abs() < self.maxdisp)
            & (flowl0[:, 1].abs() < (self.maxdisp // self.fac))
        )
        ...
        for i in range(len(ws)):
            loss += (
                ws[i]
                * torch.norm((preds["flow_preds"][i] * m - flowl0[:, :2]), 0, 1)[
                    mask
                ].mean()
            )
            m /= 2
        ...
        m = 64
        for i in range(len(oors)):
            loss += self.get_oor_loss(
                flowl0[:, :2] - up_flows[i],
                oors[i],
                (m * preds["flow_reg_maxs_preds"][i]),
                occ_mask,
            )
            m /= 2
    ...
    def get_oor_loss(self, flowl0, oor3, maxdisp, occ_mask):
        ...
        loss_oor3 = F.binary_cross_entropy_with_logits(
            oor3, oor3_gt, size_average=True, weight=weights
        )
        return loss_oor3
...
class flow_reg(nn.Module):
    ...
    def __init__(self, ent=False, maxdisp=int(4), fac=1):
        ...
        self.pool3d = nn.MaxPool3d(
            (self.wsize * 2 + 1, self.wsize * 2 + 1, 1),
            stride=1,
    ...
    def create_flow(self, size):
        ...
        flowy = np.tile(
            np.reshape(
                meshgrid[0],
                [1, 2 * self.md + 1, 2 * int(self.md // self.fac) + 1, 1, 1],
            ),
            (B, 1, 1, H, W),
        )
        flowx = np.tile(
    ...
    def forward(self, x):
        ...
        if (
            b != self.flowx.shape[0]
            or h != self.flowx.shape[3]
            or w != self.flowx.shape[4]
        ):
        ...
        if self.ent:
            ...
            return torch.cat([outx, outy], 1), torch.cat(
...
class WarpModule(nn.Module):
    ...
    def forward(self, x, flo):
        ...
        if (
            B != self.grid.shape[0]
            or H != self.grid.shape[2]
            or W != self.grid.shape[3]
        ):
...
class VCNSmall(BaseModel):
    ...
    def __init__(self, args, md=[4, 4, 4, 4, 4]):
        ...
        # c6
        self.dc6_conv1 = conv(
            128 + 4 * fdimb1, 128, kernel_size=3, stride=1, padding=1, dilation=1
        )
        ...
        self.dc6_conv7 = nn.Conv2d(
            32, 2 * fdimb1, kernel_size=3, stride=1, padding=1, bias=True
        )

        # c5
        self.dc5_conv1 = conv(
            128 + 4 * fdimb1 * 2, 128, kernel_size=3, stride=1, padding=1, dilation=1
        )
        ...
        self.dc5_conv7 = nn.Conv2d(
            32, 2 * fdimb1 * 2, kernel_size=3, stride=1, padding=1, bias=True
        )

        # c4
        self.dc4_conv1 = conv(
            128 + 4 * fdimb1 * 3, 128, kernel_size=3, stride=1, padding=1, dilation=1
        )
        ...
        self.dc4_conv7 = nn.Conv2d(
            32, 2 * fdimb1 * 3, kernel_size=3, stride=1, padding=1, bias=True
        )

        # c3
        self.dc3_conv1 = conv(
            64 + 16 * fdimb1, 128, kernel_size=3, stride=1, padding=1, dilation=1
        )
        ...
        self.dc3_conv7 = nn.Conv2d(
            32, 8 * fdimb1, kernel_size=3, stride=1, padding=1, bias=True
        )

        ## Out-of-range detection
        # if size[0]>1:  # only in train mode
        self.dc6_convo = nn.Sequential(
            conv(128 + 4 * fdimb1, 128, kernel_size=3, stride=1, padding=1, dilation=1),
        ...
        self.dc3_convo = nn.Sequential(
            conv(64 + 16 * fdimb1, 128, kernel_size=3, stride=1, padding=1, dilation=1),
            conv(128, 128, kernel_size=3, stride=1, padding=2, dilation=2),
            conv(128, 128, kernel_size=3, stride=1, padding=4, dilation=4),
            conv(128, 96, kernel_size=3, stride=1, padding=8, dilation=8),
            conv(96, 64, kernel_size=3, stride=1, padding=16, dilation=16),
    ...
    def corr(self, refimg_fea, targetimg_fea, maxdisp, fac=1):
        ...
        corr = SpatialCorrelationSampler(
            kernel_size=1,
            patch_size=(int(1 + 2 * maxdisp // fac), int(1 + 2 * maxdisp)),
            stride=1,
            padding=0,
            dilation_patch=1,
        )
    ...
    def forward(self, inputs, disc_aux=None):
        ...
        cost6 = cost6.view(
            -1, u, v, h, w
        )  # bx16, 9,9,h,w, also predict uncertainty from here
        ...
        flow5 = (flow5h.view(b, -1, 2, h, w) * F.softmax(va5, 1)).sum(
            1
        )  # b, 2k, 2, h, w
...
class VCN(VCNSmall):
    ...
    def forward(self, inputs, disc_aux=None):
        ...
        cost2 = cost2.view(
            -1, u, v, h, w
        )  # bx12, 9,9,h,w, also predict uncertainty from here
