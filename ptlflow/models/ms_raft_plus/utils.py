...
class InputPadder:
    ...
    def __init__(self, dims, mode="sintel", coarsest_scale=8):
        ...
        pad_ht = (
            ((self.ht // coarsest_scale) + 1) * coarsest_scale - self.ht
        ) % coarsest_scale
        pad_wd = (
            ((self.wd // coarsest_scale) + 1) * coarsest_scale - self.wd
        ) % coarsest_scale
        if mode == "sintel":
            self._pad = [
                pad_wd // 2,
...
def get_correlation_depth(lookup_pyramid_levels, lookup_radius):
    depth = ((2 * lookup_radius + 1) ** 2) * lookup_pyramid_levels
